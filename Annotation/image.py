...
class Image(Semantic):
    ...
    def from_path(cls, path,id=0):
        ...
        return cls(image_array, path=path,id=id)
...
#            annotation.index(self)
    ...
    def add(self, annotation, category=None):
        ...
        if isinstance(annotation, BBox):
            annotation = Annotation.from_bbox(annotation, image=None, category=category)
        ...
        if annotation.id < 0 :
           ...
           annotation.set_image(self) 
           self.annotations.append(annotation.coco(False))
           self.categories.append(annotation.category.coco(False))
